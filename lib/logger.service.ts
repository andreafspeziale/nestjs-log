...
import { InjectLogger, InjectLoggerModuleOptions } from './logger.decorators';
import {
  LoggerLevel,
  LogMetadata,
  LogLevel,
...
import { isKeyValueObject } from './logger.utils';
...
export class LoggerService implements ILoggerService {
  ...
  setContext(context: string, version?: string): void {
    this.context = version ? `${context} (version: ${version})` : `${context}`;
  }
  ...
  private redactLogMetadata(metadata: Record<string, unknown>): Record<string, unknown> {
    return Object.keys(metadata).reduce((acc: Record<string, unknown>, key: string) => {
